...
entity_id = "NHTRP7FT_LPNFT_gov"
url = "http://www.lpft.nhs.uk/about-us/accessing-our-information/annual-reports-and-accounts/expenditure_over_25k"
errors = 0
data = []
...
for block in blocks:
    try:
        if '.csv' in block['href'] or '.xls' in block['href'] or '.xlsx' in block['href'] or '.pdf' in block['href']:
            link = 'http://www.lpft.nhs.uk/'+block['href']
            title = block.text.strip().split()
            csvMth = title[0][:3]
            match = re.search(r'(\d{4})', link)
            csvYr = match.group(1)
            if '2020' in csvYr or '2025' in csvYr:
                csvYr = '2012'
